...
solution: "設計 10 階段、需逐階審批的非破壞式佈署流程：先以唯讀腳本稽核既有 CUDA 棧與模型清單，再建立獨立的 rocm-*-r9700 conda 環境、佈署 ComfyUI ROCm（埠 8189，與 CUDA 的 8188 並存），以符號連結共用 /mnt/c/ai_models 避免重複下載，並提供 Python 測試器透過 ComfyUI HTTP API 跑 SDXL／FLUX／Qwen-Image／LTX-2.3 等驗證工作流。"
...
highlights:
  ...
  - "模型共用策略：以符號連結映射 /mnt/c/ai_models，避免數十 GB 權重重複佔用磁碟"
